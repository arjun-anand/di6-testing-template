...
describe('Hotel Booking',()=>{
    describe('Hotel Booking Constructor',()=> {
        it.skip('should be a constructor for the Hotel Booking class',()=> {
            expect(new HotelBooking()).to.be.an.instanceOf(HotelBooking)
        })
        it.skip('should allow the specification of a Name in the constructor',()=>{
            expect(new HotelBooking('Ada Hotel')).to.have.property('name')
        })
        it.skip('should allow the specification of an City in the constructor',()=>{
   ...
   describe('Holiday Properties',() => {
    ...
    describe('Hotel Methods',()=>{
        describe('Price Per Night',()=>{
            it.skip('should return the base price for days that are not Weekends or Summer',() => {
                let hotel = new HotelBooking('Ada Hotel','London')
                let date = new Date(2020,2,5)
                hotel.basePrice = 100
                expect(hotel.pricePerNight(date)).to.equal(100)
            })
            it.skip('should return the base price with a 20% increase for Weekends', ()=> {
                let hotel = new HotelBooking('Ada Hotel','London')
                let date = new Date(2020,2,1)
                hotel.basePrice = 100
                expect(hotel.pricePerNight(date)).to.equal(120)
            })
            it.skip('should return the base price with a 50% increase for Summer', ()=> {
                let hotel = new HotelBooking('Ada Hotel','London')
                let date = new Date(2020,6,1)
                hotel.basePrice = 100
                expect(hotel.pricePerNight(date)).to.equal(150)
            })
            it.skip('should return the base price with a combined increase for Summer Weekends', ()=> {
                let hotel = new HotelBooking('Ada Hotel','London')
        ...
        describe('should calculate the price per Trip',()=>{
            it.skip('should be 100 for a single winter weekday',() => {
                let hotel = new HotelBooking('Ada Hotel','London')
                hotel.basePrice = 100
            ...
            it.skip('should be 240 for a two winter weekend days',() => {
                let hotel = new HotelBooking('Ada Hotel','London')
                hotel.basePrice = 100
            ...
            it.skip('should be 450 for a three summer week days',() => {
                let hotel = new HotelBooking('Ada Hotel','London')
                hotel.basePrice = 100
